...
export const GoogleSheetSyncModal: React.FC<GoogleSheetSyncModalProps> = ({
  ...
  const [appsScriptUrl, setAppsScriptUrl] = useState<string>(() => {
    return safeStorage.getItem('gp_apps_script_url') || PERMANENT_APPS_SCRIPT_URL;
  });
  ...
  const handleSavePermanently = async (syncNow: boolean = true) => {
    ...
    let targetSheetUrl = trimmedUrl;
    let targetAppsScriptUrl = appsScriptUrl;

    if (trimmedUrl.includes('script.google.com')) {
      targetAppsScriptUrl = trimmedUrl;
      targetSheetUrl = PERMANENT_BATHUARY_SHEET_URL;
      setSheetUrl(PERMANENT_BATHUARY_SHEET_URL);
      setAppsScriptUrl(trimmedUrl);
      safeStorage.setItem('gp_apps_script_url', trimmedUrl);
    }
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-2xl bg-slate-900 text-slate-200 space-y-3 shadow-inner">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    const scriptCode = MOBILE_APP_APPS_SCRIPT_CODE;
